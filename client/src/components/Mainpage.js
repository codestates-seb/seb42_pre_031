...
// 모든 글이 뿌려지는 컴포넌트..
function Mainscript({ data, setData, data2, tureFalse,  }) {
  const HH = data2.map((a,i)=>{ return data2[i]})
  const lastsort = HH.sort((a, b) =>  new Date(b.createdAt) - new Date(a.createdAt) );
  return (
    tureFalse ?
    <>
      {data.map((a,i) => {
